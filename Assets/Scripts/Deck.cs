...
public class Deck : MonoBehaviour
{
    ...
    public List<GameObject> drawed; //A lista dos prefabs que est�o na m�o do player
    public List<GameObject> hand; //A lista dos objetos instaciados que est�o na m�o do player
    public List<int> get; //O index das cartas que est�o na m�o do player
    public GameObject deckObject; //O objeto do deckp1 da cena
    public GameObject deckSelectButton; //Os bot�es da sele��o de deck da cena
    ...
    public GameObject actionHud; //O hud de a�oes (Walk, rock, etc.)
    ...
    public List<GameObject> cardPlaces; //As posi��es das cartas na tela
    public List<GameObject> attackAreas; //As areas que ser�o atacadas no campo do oponente
    public bool decided; //Verifica se o player ja decidiu o deck
    public bool choosed; //Verifica se o player ja escolheu uma carta
    private int count;
    public bool deckFull; //Verifica se o deck embaralhado j� esta cheio ou n�o
    ...
    void Update()
    {
       /* if (!decided)
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0)) //Se o bot�o esquerdo for clicado, ativa a fun��o de usar os objetos
        {
            cc.ClickToUse();
        }
        if (Input.GetKeyDown(KeyCode.Mouse1)) //Se o bot�o direito for clicado, ativa a fun��o de ver os objetos
        {
            cc.ClickToView();
        }
       ...
       /* if(Input.GetKeyDown(KeyCode.R) && drawed.Count < 5)
        ...
        foreach(GameObject a in hand) //Pra cada carta na m�o, ele coloca a carta na posi��o da tela equivalente ao seu index, e ve se o player tem mana pra usar ela ou n�o alterando o tamanho dela dependendo do resultado
        {
    ...
    IEnumerator deckDefine(List<GameObject> deckBase) //Fun��o que embaralha o deck selecionado e entrega a primeira m�o
    {
        ...
        while (!deckFull) //Enquanto o deck embaralhado n�o estiver cheio ele pega uma carta aleatoria do deck original e joga pro embaralhado 
        {
         ...
         foreach (GameObject i in deck) //Enquanto o numero de cartas na mao n�o for 5, ele puxa a primeira carta do deck e adiciona ela na lista de prefabs, depois instancia ela na tela e adiciona o objeto novo na lista dos instaciados
         {
                if(drawed.Count == 5)
                {
                    break;
        ...
        while (deck.Count > 25) //Enquanto ele n�o tiver removido 5 cartas do deck, ele remove as cartas que forem puxadas do deck
        {
    ...
    public IEnumerator useCard(GameObject g) //A fun��o que usa a carta selecionada
    {
        int rnd = Random.Range(0, 4);
        print("ta funcionando");
        if (!use) //Verifica se nenhuma carta ja foi usado, caso n�o: 
        {
               ...
               if (temp != null && temp.GetComponent<Cards>().mana <= jm.P1_MANA) //Ve se tem mana pra usar ela
               {
                    ...
                    player.GetComponent<Animator>().SetTrigger(temp.GetComponent<Cards>().trigger); //Ativa a anima��o dela
                ...
                if (!teleport) //Se teleporte ainda n�o tiver sido usado: 
                {
                    if (demo.GetComponent<Cards>().isAttack) //Verifica se � uma carta de ataque, se for: 
                    {
                        ...
                        for (int i = 0; i < demo.GetComponent<Cards>().attackAreaP1.Count; i++) //Adiciona todas as areas de ataque da carta na lista das areas que v�o ser atacadas
                        {
                    ...
                    else if (demo.GetComponent<Cards>().isAttackBuff) //Se for uma carta de buff de ataque
                    {
                        ...
                        jm.p1damage = 0; //Diz que o dano � 0
                        print("buff");
                    }
                    ...
                    //Destroy(g);
                    draw = false; //Diz que o player ainda n�o comprou uma carta
                    choosed = false; //Diz que o player ainda n�o escolheu uma carta
    ...
    public void SelectCard(GameObject g) //Fun��o ativa quando o player seleciona a carta na m�o dele
    {
        print("clicou em: " + g.name);
        if (!choosed) //Ve se ele ja nao escolheu uma carta, caso n�o:
        {
            ...
            if (temp != null && temp.GetComponent<Cards>().mana <= jm.P1_MANA) //Ve se tem mana suficiente
            {
                ...
                actionHud.gameObject.SetActive(false); //Desativa o hud de a�oes
                ...
                for (int i = 1; i < 10; i++) //Desativa todos as casas do tabuleiro do inimigo, pra n�o bugar caso varias cartas sejam selecionadas em sequencia
                {
                    b.EnemyPositions[i].GetComponent<MeshRenderer>().enabled = false;
                    b.EnemyPositions[i].GetComponent<BoxCollider>().enabled = false;
                }
                if (g.GetComponent<Cards>().isAttack) //Ve se � uma carta de ataque, se for: 
                {
    ...
    public void DrawCard() //Fun��o que faz a compra da carta no inicio do turno, funciona igual a de definir deck mas s� pra uma carta inves de 5
    {
   ...
   /* public void fireDeck() //Escolhe o deck de fogo
    ...
    public void skipTurn() //Pula o turno, zerando todos os valores e dizendo que o player fez uma a��o, ent�o mandando o inimigo escolher uma carta
    {
        draw = false;
        use = true;
        jm.p1damage = 0;
    ...
    public void SummonCard() //Fun��o do bot�o summon
    {
    ...
    public void Back() //Fun��o do bot�o de voltar da carta
    {
